...
这是引自gulp中文网上的一句话，通俗来讲，gulp就是帮助完成一些前端的重复性工作（编译sass/scss/less/stylus，压缩css，压缩js，合并css，合并js，压缩image文件等），并且极大的提高工作效率（自动合并css，js文件优化http请求，自动添加md5后缀强制清空浏览器缓存等）。

PS：具有相同作用的工具还有grunt和fis3

<!-- more -->

## 项目实例

### [安装gulp](http://www.gulpjs.com.cn/docs/getting-started/)
...
```
### 注意：
* 配置的大部分功能都在注释中有所说明，实际运用中类似合成sprite，合并css，合并js的功能一般不会只有一个，可根据需求增加task设置。
* 代码中有几项需要配合额外文件使用，在此列出，可以借助最下方demo做实际演示：
	* `gulp.spritesmith`中的`cssTemplate`参数，本次使用的是mustache语法
	* `gulp-scss-lint`中代码规范`scsslint.yml`
	* `gulp-useref`中合并资源的替换需要在html中使用特定的注释语法

### [demo page](https://github.com/jinming6568/gulp-project)
